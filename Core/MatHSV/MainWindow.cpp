...
void MainWindow::_updateAlgFunction() {
    ...
    alg_=OpenCVImageItem::AlgFunctionType(
        ...
        [sh_,ss_,sv_](const QImage & image_)->QImage {
        ...
        cv::Mat cvHSV_;
        cv::cvtColor(cvMat_.first,cvHSV_,CV_RGB2HSV);
        std::vector<cv::Mat> hsv_;
        cv::split(cvHSV_,hsv_);
        if(hsv_.size()>=3){
            hsv_[0]*=sh_;hsv_[1]*=ss_;hsv_[2]*=sv_;
            cv::merge(hsv_,cvHSV_);
            cv::cvtColor(cvHSV_,cvMat_.first,CV_HSV2RGB);
