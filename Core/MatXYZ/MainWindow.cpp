...
MainWindow::MainWindow(QWidget *parent)
    ...
    typedef void(QDoubleSpinBox::* TT)(double );
    connect(cwidget_->X_.spinBox_,TT(&QDoubleSpinBox::valueChanged),
        this,[this](double) {updateAlgFunction(); });
    connect(cwidget_->Y_.spinBox_,TT(&QDoubleSpinBox::valueChanged),
        this,[this](double) {updateAlgFunction(); });
    connect(cwidget_->Z_.spinBox_,TT(&QDoubleSpinBox::valueChanged),
        this,[this](double) {updateAlgFunction(); });
}
...
void MainWindow::_updateAlgFunction() {
    ...
    /*获得xyz缩放值*/
    double sx_,sy_,sz_;
    sx_ = cw_->X_.spinBox_->value();
    sy_ = cw_->Y_.spinBox_->value();
    sz_ = cw_->Z_.spinBox_->value();

    /*生成函数*/
    alg_=OpenCVImageItem::AlgFunctionType(
        new std::function< QImage(const QImage &) >{
        [sz_,sx_,sy_](const QImage & image_)->QImage {
        ...
        cv::Mat cvXYZ_;
        cv::cvtColor(cvMat_.first,cvXYZ_,CV_RGB2XYZ);
        std::vector<cv::Mat> xyz_;
        cv::split(cvXYZ_,xyz_);
        if(xyz_.size()>=3){
            xyz_[0]*=sx_;xyz_[1]*=sy_;xyz_[2]*=sz_;
            cv::merge(xyz_,cvXYZ_);
            cv::cvtColor(cvXYZ_,cvMat_.first,CV_XYZ2RGB);
